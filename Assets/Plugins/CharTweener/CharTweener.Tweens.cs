...
namespace CharTween
{
    public partial class CharTweener : MonoBehaviour
    {
        ...
        public Tweener DOLocalCircle(int charIndex, float radius, float duration, int pathPoints = 8, PathType pathType = PathType.CatmullRom,
            PathMode pathMode = PathMode.Full3D, int resolution = 10, Color? gizmoColor = null)
        {
            var tweenPath = new Vector3[pathPoints + 1];
            for (var i = 0; i < tweenPath.Length; ++i)
            ...
            tweenPath[tweenPath.Length - 1] = new Vector3(radius, 0, 0);
            SetLocalPositionOffset(charIndex, tweenPath[0]);
