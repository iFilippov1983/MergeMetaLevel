//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by Entitas.Roslyn.CodeGeneration.Plugins.CleanupSystemsGenerator.
//
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------
public sealed class ChipsCleanupSystems : Feature {

    public ChipsCleanupSystems(Contexts contexts) {
        Add(new DestroyDeleteChipsSystem(contexts));
        Add(new RemoveVisitedChipsSystem(contexts));
        Add(new RemoveDoMoveChipsSystem(contexts));
        Add(new RemoveClickChipsSystem(contexts));
    }
}
